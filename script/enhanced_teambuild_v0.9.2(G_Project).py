...
queue = Queue.Queue()
f_mergedCommits = open(os.getcwd()+'/mergedCommits.txt', 'w')
...
def getChanges():
    ...
    qcmd = ["ssh", "-p", "29447", "165.243.137.64", "gerrit", \
	   ...
	   "status:open", \
           "label:CodeReview+2", "label:Verified+1", \
...
def getChangesByTime(a_changes, start_time, end_time):
    changes = []

    # start_datetime = start_time # now stat_time is none
    start_datetime = 0
    if end_time == None :
        print 'must input end_time'
        return
    else:
        # localserver time - korean time + fix time
        timeFormat = '%Y-%m-%d  %H:%M:%S'
        end_datetime = time.mktime(time.strptime(end_time, timeFormat )) -  9*60*60 + (-time.timezone)
    
    cond_commit = u'createdOn' # u'lastUpdated' is update time/ Now we use commit create time
    f = open("allCommit.txt", 'w')

    for change in a_changes:
        if change[cond_commit] >= start_datetime and change[cond_commit] <= end_datetime:
...
def checkApplyError(outString, change):
    ...
    if outString.find(errorCase1) != -1:
        ...
        print change[u'currentPatchSet'][u'revision']
        global f_mergedCommits
        f_mergedCommits.write(change[u'currentPatchSet'][u'revision'] + "\n")
        f_mergedCommits.flush()
    return 0
...
def main(argv):
   os.system("rm -rf Process-*")  # clean Process log file
   start_time = 0
   end_time = os.environ.get("COMMIT_ENDTIME")
   if end_time == None:
       print 'No COMMIT_ENDTIME, So exit!!!'
       return  

   changes = getChangesByTime(getChanges(), start_time, end_time )
   listChangeSet = devidebyProject(changes)
